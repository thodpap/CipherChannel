...
Recent research systems have also explored wireless exoskeleton-control interfaces, while clinical lower-limb studies demonstrate assisted mobility functions such as standing, stepping, and ambulation in rehabilitation settings~\cite{andersson2024wireless,atlas2030_2022}. 
...
The maximum plaintext command size was 400B, producing a maximum CipherChannel packet size of 428B after adding the 12-byte counter nonce and 16-byte AES-GCM authentication tag. Unless otherwise noted, reported latency values and percentiles were computed across the recorded trials using linear-interpolation percentiles (Hyndman--Fan Type~7). For the laptop/Raspberry~Pi latency tables, Tables~\ref{tab:steady_latency}--\ref{tab:distance_steady} additionally report a 95\% confidence interval obtained by percentile bootstrap (10,000 resamples with replacement over the per-trial measurements). The ESP32 hardware-in-the-loop table reports percentiles but not bootstrap intervals because it is a single endpoint run with a smaller trial count.
