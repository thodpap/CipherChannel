...
CipherChannel passed all protocol-correctness and provisioning-gate tests, and encrypted command exchange achieved 169.6\,ms median and 214.7\,ms p99 client-observed command-to-ACK latency.
\end{abstract}
...
Contemporary commercial and research systems increasingly expose wireless supervisory interfaces so that clinicians or users can initiate discrete gait events such as standing, walking, stopping, or readiness transitions without a tethered operator station~\cite{andersson2024wireless,atlas2030_2022}. 
...
\begin{itemize}
  ...
  \item \textbf{CipherChannel protocol}: We design and formalise CipherChannel, an application-layer protocol that combines AES-256-GCM, persistent 96-bit monotonic counter nonces, direction-separated parity classes, crash-safe counter persistence, and fail-closed receive semantics. The protocol avoids wall-clock freshness windows, unauthenticated reset messages, and BLE-session-dependent replay state.
  ...
  \item \textbf{Empirical validation under measured BLE conditions}: We evaluate the current reference implementation through protocol-correctness tests, provisioning-gate tests, adversarial attempts, cross-endpoint injection attempts, steady-state BLE command trials, cold-start trials, key-exchange trials, fixed-distance obstruction trials, and an exploratory two-wall range-to-failure run. The results support the replay-resistance and practical latency claims for sparse supervisory commands, while also delimiting claims that remain unvalidated on C/OpenSSL, ESP32/mbedTLS, RPi reboot recovery, and long-duration multi-client deployments.
\end{itemize}

The remainder of the paper develops this argument in stages. Section~\ref{sec:background} reviews the BLE security weaknesses, exoskeleton-control assumptions, and application-layer security approaches that motivate protection above the BLE link layer. Section~\ref{sec:sysarch} then introduces the gateway-mediated architecture used to separate wireless connectivity from runtime command acceptance. Building on this architecture, Section~\ref{sec:protocol} specifies the CipherChannel protocol, including its threat model, packet format, counter-nonce replay protection, key architecture, provisioning mechanism, and reset-free operation. Section~\ref{sec:impl} describes the reference implementation used in the prototype, while Section~\ref{sec:eval} presents the experimental evaluation, including protocol-correctness, adversarial-rejection, endpoint-isolation, latency, processing-cost, key-exchange, and distance/obstruction results. Section~\ref{sec:discussion} discusses the implications of these results, the deployment trade-offs, the remaining limitations, regulatory alignment, and future work. Section~\ref{sec:conclusion} concludes the paper.
...
Commercial and research systems expose discrete operational states or commands such as standing, walking, stepping, stopping, or readiness transitions~\cite{vandijsseldonk2021predictors,lee2021locomat,atlas2030_2022}.
...
On reception, the receiver first verifies the AES-GCM tag. If authentication succeeds, it interprets the nonce as a little-endian integer $c$ and accepts the message only if:

\begin{equation}
c > \mathit{ctr}_{recv}
\quad\land\quad
(c \bmod 2) = (\mathit{ctr}_{recv} \bmod 2).
...
\begin{itemize}
  ...
  \item \textbf{Operational Key} ($K_S$): a deployment-specific 256-bit key used for authenticated encryption of operational command traffic. Authorised endpoints obtain $K_S$ during a physically gated exchange, and the gateway maintains separate cipher contexts for the supervisory-client path and the cane-controller path. Thus, send counters, receive counters, parity roles, and replay state are isolated per endpoint even when the same operational key material is used.
\end{itemize}

This separation ensures that BLE pairing success is not sufficient for command authorisation. An accepted command must be protected under $K_S$ and must satisfy AEAD verification, counter-monotonicity validation, and direction-parity checks. A stricter production deployment can replace the shared deployment key with endpoint-specific operational keys ($K_{S,phone}$ and $K_{S,cane}$) without changing the packet format; this is discussed as a hardening step in Section~\ref{sec:limitations}.

\subsection{Physical-Presence-Gated Key Exchange}
\label{sec:keyexchange}

Key exchange is the process by which a client acquires $K_S$ from the
gateway over BLE. The protocol is described in
...
\footnotesize
\begin{algorithmic}[1]
\Require Client and gateway share $K_T$; gateway holds $K_S$
\Ensure  Client acquires $K_S$ encrypted under $K_T$
...
Consequently, a captured provisioning ciphertext cannot be replayed successfully in a later provisioning window, because the previous window state has been discarded and a fresh provisioning interaction is required.
...
The reference implementation includes both the Python/PyCryptodome gateway implementation and an ESP32/mbedTLS endpoint implementation. The packet format is library-independent and is defined by the \texttt{nonce || ciphertext || tag} layout, the 96-bit counter nonce, and the receive-side monotonicity and direction-parity checks. The quantitative evaluation reported in this paper focuses on the Python/Raspberry~Pi BLE gateway path; broader cross-platform interoperability testing with additional endpoints is left for extended validation. The reference implementation and experiment artefacts are publicly available under the Apache-2.0 license at \url{https://github.com/thodpap/CipherChannel}.
...
The canonical evaluation covers the Python/PyCryptodome implementation, the Raspberry~Pi BLE gateway path, the provisioning-gate state machine, and BLE client-to-gateway command exchange. It does not evaluate C/OpenSSL interoperability, ESP32/mbedTLS interoperability, physical Raspberry~Pi power-cycle recovery, ESP32 resource usage, long-duration soak behaviour, or clinical workflow execution. These aspects are therefore treated as limitations rather than validated results.
...
A separate endpoint-isolation experiment tested whether packets from one logical endpoint path could be accepted by the other. The gateway performed cane and phone key exchange, then injected 30 phone-encrypted packets into the cane channel and 30 cane-encrypted packets into the phone channel. All 60 cross-endpoint injections were rejected, and one valid command per channel was accepted as a sanity check. In this canonical run, rejection occurred at AES-GCM authentication because the phone and cane paths used distinct operational keys. This result supports endpoint isolation for the tested gateway configuration.
...
\centering
\footnotesize % Increased from \scriptsize for better readability
\begin{tabularx}{\columnwidth}{@{} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{4cm} @{}}
\toprule
\textbf{Metric} & \textbf{Observed value} \\
\midrule
Total flash / ROM usage with CipherChannel & 1,155,721~B / 1,310,720~B (88\%) \\
Total flash / ROM usage without CipherChannel & 1,147,437~B / 1,310,720~B (87\%) \\
Estimated CipherChannel flash increment & 8,284~B, i.e., approx.\ 8.09~KiB \\
Dynamic RAM usage with CipherChannel & 38,748~B / 327,680~B (11\%) \\
Dynamic RAM usage without CipherChannel & 38,748~B / 327,680~B (11\%) \\
Estimated CipherChannel dynamic-RAM increment & 0~B \\
Free RAM remaining & 288,932~B \\
Minimum free stack & 4,312 words, i.e., approx.\ 17,248~B \\
Measurement method & Arduino build output and FreeRTOS stack high-water mark \\
\midrule
\multicolumn{2}{@{}p{\columnwidth}@{}}{\scriptsize \textit{Note:} Total firmware values include the Arduino core, BLE support code, mbedTLS, platform support code, NVS handling, and application logic. The CipherChannel-specific flash increment was estimated by compiling the same ESP32 BLE cane firmware with CipherChannel enabled and disabled and subtracting the Arduino build-reported program-storage usage. No increase in Arduino build-reported global dynamic-RAM usage was observed. Stack usage was measured at runtime using the FreeRTOS stack high-water mark during BLE and CipherChannel operation.} \\
\bottomrule
\end{tabularx}
\end{table}
...
The latency results also support the practical feasibility claim, but with a narrower interpretation than a general real-time-control claim. CipherChannel is appropriate for sparse supervisory commands such as \texttt{STAND\_UP}, \texttt{START\_WALKING}, \texttt{TAKE\_STEP}, \texttt{STOP\_ACTION}, and \texttt{READY\_FOR\_ACTIONS}. It is not evaluated as a high-rate joint-control or servo-loop security mechanism. In steady-state BLE operation, the median command-to-ACK latency was 169.6\,ms and the p99 latency was 214.7\,ms. These values are in the range of simple human reaction-time measurements often reported for visual and auditory stimuli~\cite{kosinski2013reaction}, but this comparison should be interpreted cautiously. It supports plausibility for clinician- or user-initiated supervisory control; it does not establish suitability for closed-loop motion control.

\subsection{Latency Trade-offs}

The main latency trade-off is not AES-GCM encryption itself. Client-side encryption had 0.215\,ms median latency in the steady-state experiment, and gateway-side AES-256-GCM authentication and decryption had 1.70\,ms median latency. The command-to-ACK path was dominated by BLE write/ACK timing and by local receive-counter persistence on the gateway. The persistence result is particularly important: the fsync-based receive-counter update had 14.21\,ms median and 101.87\,ms p99 latency. This is the cost of enforcing crash-safe replay protection with synchronous durable state updates.
...
The provisioning gate adds an operational safety control that is distinct from BLE pairing. The button-press gate on key exchange is related in spirit to proximity-gated access-control mechanisms studied in implantable-medical-device work~\cite{chi2018esafe}, but it uses an explicit operator action rather than only RF proximity inference. In the exoskeleton context, requiring a clinical staff member or authorised operator to enable the provisioning window provides a simple out-of-band authorisation step before an endpoint can acquire operational key material, and is consistent with networked medical-device risk-management expectations~\cite{iec80001}.

The canonical run validates the provisioning-gate state machine, not the complete physical button implementation. Therefore, the result supports the software logic for one-shot windows, timeout, endpoint separation, and closed-gate rejection, while the physical interaction itself still requires hardware-in-the-loop validation.

\subsection{Distance, Obstruction, and Deployment Constraints}

The distance experiments showed 100\% success under the tested line-of-sight and wall-obstructed conditions. In steady-state operation, median latency stayed close to 169\,ms across 1\,m, 2\,m, 4\,m, and 7\,m-wall conditions. The main change under obstruction was increased variance and tail latency, with the 7\,m-wall condition reaching 259.6\,ms p99 and 350.5\,ms maximum command-to-ACK latency. In the progressive two-wall walk-back run, the system completed 43 successful command/ACK exchanges before an ACK-read failure and subsequent connection loss near approximately 10m through two walls.

This result should not be overgeneralised. The experiments were conducted with a specific laptop adapter, Raspberry~Pi gateway, BLE stack, physical environment, wall geometry, and command payload. They indicate that the tested configuration remains usable across the measured operating distances and exposes an approximate edge-of-range failure point under two-wall obstruction. They do not establish performance in clinics, homes, crowded RF environments, mobile-phone BLE stacks, multi-client contention, mobility scenarios, or on-body antenna orientations. Those conditions require separate validation, particularly for timeout handling, reconnection behaviour, and fail-safe user feedback after link loss.

\subsection{Limitations}
\label{sec:limitations}

The evaluated prototype has three main limitations. First, authorised endpoints currently share one operational key; separate cipher contexts isolate counter and replay state, but disclosure of one endpoint would compromise the shared deployment key. Production deployments should therefore provision endpoint-specific operational keys using the same physical-presence-gated mechanism. Second, the bootstrap transport key $K_T$ is static. Although it is used only during provisioning, a stronger design should replace it with an ephemeral authenticated key-establishment mechanism, such as ECDH with device certificates or hardware-bound credentials. Third, CipherChannel provides command authentication, integrity, confidentiality, replay protection, and crash-safe counter persistence, but it does not address denial-of-service attacks such as radio jamming, GATT flooding, or gateway resource exhaustion. These availability attacks remain outside the present threat model.

\subsection{Regulatory Alignment}
\label{sec:regulatory}

CipherChannel is not a complete regulatory compliance framework, but it aligns with current medical-device cybersecurity expectations. FDA's 2023 guidance requires secure development, threat modelling, SBOM documentation, vulnerability management, and NIST-approved cryptographic controls~\cite{fda2023guidance}; IEC~81001-5-1:2021 and IEC~TR~60601-4-5:2020 similarly emphasize secure communication and authenticated encryption for safety-relevant medical-device software~\cite{iec81001,iec60601_4_5}. CipherChannel addresses the protocol-level communication requirement through AES-256-GCM, persistent replay protection, endpoint isolation, and physical-presence-gated key provisioning before commands reach the exoskeleton runtime.
...
\bibitem{giansanti2021cybercare}
D.~Giansanti and R.~A.~Gulino, ``The Cybersecurity and the Care Robots: A Viewpoint on the Open Problems and the Perspectives,'' \emph{Healthcare}, vol.~9, no.~12, art.~1653, 2021, doi: \url{https://doi.org/10.3390/healthcare9121653}.

\bibitem{fosch2021cyberrobot}
E.~Fosch-Villaronga and T.~Mahler, ``Cybersecurity, Safety and Robots: Strengthening the Link between Cybersecurity and Safety in the Context of Care Robots,'' \emph{Computer Law \& Security Review}, vol.~41, art.~105528, 2021, doi: \url{https://doi.org/10.1016/j.clsr.2021.105528}.
