...
Despite this risk, existing work does not provide a protocol level treatment of application layer command authentication for BLE mediated lower limb exoskeleton control. The missing problem is not only encryption over BLE, but the acceptance rule for safety relevant commands: when should a gateway release a received wireless command to a runtime that can move a device attached to a human body?
...
The system is built around application-layer authenticated encryption, counter-based replay protection, physical-presence provisioning, and controlled forwarding into the proprietary runtime. It also keeps the supervisory and cane endpoints separate. The Python/PyCryptodome reference implementation has been validated, while C/OpenSSL and ESP32/mbedTLS versions remain future portability targets.
...
\begin{itemize}
  ...
  \item \textbf{RQ3 -- Measured command latency}: Under the tested BLE conditions, what latency is observed for steady state command-to-ACK exchange, cold start operation, key exchange, and distance/obstruction scenarios?
...
\caption{Steady State Secure command-to-ACK Latency for \texttt{STAND\_UP} (ms, $N=1000$)}
...
Table~\ref{tab:coldstart} reports the cold start result. Across 500 trials, all commands succeeded. The total cold start path had 4.62\,s median latency and 10.38\,s p99 latency. The command-to-ACK part of the same cold start trials had 179.5\,ms median and 224.9\,ms p99 latency. The difference shows that scan, connection establishment, and key exchange dominate cold start operation, while the encrypted command-to-ACK exchange remains in the same order of magnitude as the steady state case.
...
\toprule
\textbf{Phase} & \textbf{p50} & \textbf{Mean} & \textbf{p90} & \textbf{p95} & \textbf{p99} & \textbf{Max} \\
\midrule
Socket setup            &  0.03 &   0.04 &   0.04 &   0.20 &    0.25 &    0.29 \\
DTLS handshake (PSK)    & 22.52 & 118.52 & 235.25 & 502.75 & 1181.85 & 4722.85 \\
Command$\rightarrow$ACK &  3.69 &  30.81 &  59.88 & 114.69 &  478.14 & 2722.71 \\
\midrule
\textbf{Total}          & \textbf{27.62} & \textbf{149.39} & \textbf{329.73} & \textbf{740.15} & \textbf{1424.14} & \textbf{5686.97} \\
\bottomrule
\end{tabular}
\end{table}
...
These DTLS results should not be compared directly against the CipherChannel BLE cold-start total. The CipherChannel cold-start median in Table~\ref{tab:coldstart} is dominated by BLE scan and BLE connection establishment, not by CipherChannel cryptographic processing. Once the BLE connection exists, the CipherChannel command-to-ACK median is 179.5\,ms, reflecting BLE scheduling and durable receive counter persistence. A DTLS over GATT design would still be expected to incur the BLE scan and connection costs observed in Table~\ref{tab:coldstart}, while adding a secure-channel establishment step unless session resumption or another recovery mechanism were implemented. The relevant conclusion is therefore not that CipherChannel is faster than DTLS over UDP/IP, but that CipherChannel avoids adding a second cryptographic handshake to the BLE reconnect path while preserving application-managed, crash-safe replay state for command admission.
...
% ---------------------------------------------------------------
\begin{IEEEbiography}
[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{thodoris.png}}]
{Theodoros Frixos Paparrigopoulos}
received the Diploma degree in Electrical and Computer Engineering from the National Technical University of Athens, Greece, where he is currently pursuing the Ph.D. degree. He is a software engineer and researcher with experience in the design and development of secure, data-intensive, and distributed systems. He has contributed to national and European research and innovation projects in areas including critical infrastructure resilience, climate-risk assessment, cybersecurity, information analysis, and assistive technologies. 
\end{IEEEbiography}
