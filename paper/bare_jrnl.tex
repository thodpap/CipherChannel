...
The reference implementation includes a Python/PyCryptodome gateway implementation and an ESP32/mbedTLS cane-endpoint implementation. The packet format is library-independent and is defined by the \texttt{nonce || ciphertext || tag} layout, the 96-bit counter nonce, and the receive-side monotonicity and direction-parity checks. The quantitative BLE latency and adversarial-rejection evaluation reported in this paper focuses on the Python/Bleak client and Python/Raspberry~Pi gateway path. The ESP32 implementation is evaluated only for firmware resource footprint in Section~\ref{sec:esp32-footprint}; full ESP32/mbedTLS interoperability and latency validation are left for extended testing. The reference implementation and experiment artefacts are publicly available under the Apache-2.0 license at \url{https://github.com/thodpap/CipherChannel}.
