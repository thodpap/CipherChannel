...
These DTLS results should not be compared directly against the CipherChannel BLE cold-start total. The CipherChannel cold-start median in Table~\ref{tab:coldstart} is dominated by BLE scan and BLE connection establishment, not by CipherChannel cryptographic processing. Once the BLE connection exists, the CipherChannel command$\rightarrow$ACK median is 179.5\,ms, reflecting BLE scheduling and durable receive-counter persistence. A DTLS-over-GATT design would still be expected to incur the BLE scan and connection costs observed in Table~\ref{tab:coldstart}, while adding a secure-channel establishment step unless session resumption or another recovery mechanism were implemented. The relevant conclusion is therefore not that CipherChannel is faster than DTLS over UDP/IP, but that CipherChannel avoids adding a second cryptographic handshake to the BLE reconnect path while preserving application-managed, crash-safe replay state for command admission.
